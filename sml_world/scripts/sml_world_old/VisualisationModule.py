...
class VisualisationModule:
    ...
    def __init__(self, map_filename, window_width, window_height, pixel_per_meter = -1, ground_projection = False):
        ...
        self.udp_receiver = VisualisationModuleReceiver.VisualisationModuleReceiver(self)

        self.start_thread_main_loop()

    def start_thread_main_loop(self):
        '''
        The loop that will until the user closes
        the visualisation window
        '''

        while True:

            is_quit = self.loop_iteration()

            # This is trigerred if the user
            # closed the Visualisation Window
            if is_quit:
    ...
    def loop_iteration(self):
        ...
        self.udp_receiver.process_incoming_packets()
        ...
        time_to_sleep = 1./self.refresh_rate - time_elapsed
        ...
        if time_to_sleep > 0:
            ...
            #print "VisualisationModule failed desired rate."
            pass
    ...
    def load_smart_car_image(self):
        ...
        self.smart_car_image = self.get_car_image('resources/carSmartOffset.png', car_width_meters, car_length_meters)
    ...
    def load_red_car_image(self):
        ...
        self.red_car_image = self.get_car_image('resources/carRedOffset.png', car_width_meters, car_length_meters)
    ...
    def load_green_car_image(self):
        ...
        self.green_car_image = self.get_car_image('resources/carGreenOffset.png', car_width_meters, car_length_meters)
    ...
    def load_blue_car_image(self):
        ...
        self.blue_car_image = self.get_car_image('resources/carBlueOffset.png', car_width_meters, car_length_meters)
    ...
    def load_white_car_image(self):
        ...
        self.white_car_image = self.get_car_image('resources/carWhiteOffset.png', car_width_meters, car_length_meters)
    ...
    def load_yellow_car_image(self):
        ...
        self.yellow_car_image = self.get_car_image('resources/carYellowOffset.png', car_width_meters, car_length_meters)
    ...
    def load_truck_image(self):
        ...
        self.truck_image = self.get_car_image('resources/truckTopOffset.png', truck_width_meters, truck_length_meters)
    ...
    def smart_background_blit(self):
        ...
        for area_to_blit in self.areas_to_blit:
            ...
            top_left_x = area_to_blit[0]
            top_left_y = area_to_blit[1]
            bot_right_x = area_to_blit[2]
            bot_right_y = area_to_blit[3]

            surface_to_blit_pixel_array = self.bg_surface_pixel_array[top_left_x:bot_right_x, top_left_y:bot_right_y]

            if surface_to_blit_pixel_array:
                pass
                # Not all vehicles should be blitted
                # Issue #13 Minor Bug

                #self.window.blit(surface_to_blit_pixel_array.make_surface(), (top_left_x, top_left_y))

        # Empty the areas to blit, since they were all processed
        self.areas_to_blit = []
    ...
    def draw_vehicles(self):
        ...
        for vehicle_id in self.vehicles_dict:
            ...
            elif vehicle_class_name == dummyvehicle.DummyVehicle.__name__:
        ...
        for vehicle_id in self.vehicles_dict:
            ...
            if self.show_pair:
                self.draw_pairing_information(vehicle_id)
    ...
    def draw_arrow(self,p1,p2):
        ...
        rect = (0,0,0,0)#pygame.draw.polygon(self.window, (255,0,0), point_list,1)

        surf = None #self.window.subsurface(rect)
        m1 = min(p1[0],p2[0])
        m2 = min(p1[1],p2[1])
        self.add_surface_to_areas_to_blit(surf,(m1,m2))

        
    def draw_pair(self, id1,id2):
        '''
        Draw an arrow between id1 and id2
        '''

        vehicle = self.vehicles_dict[id1]
        [p1_x, p1_y] = self.convert_position_to_image_pixel(vehicle['x'], vehicle['y'])
        vehicle = self.vehicles_dict[id2]
        [p2_x, p2_y] = self.convert_position_to_image_pixel(vehicle['x'], vehicle['y'])

        self.draw_arrow((p1_x,p1_y),(p2_x,p2_y))

     
    def draw_pairing_information(self,id):
        vehicle = self.vehicles_dict[id]

        if isinstance(vehicle['fwd'],int):
            self.draw_pair(id,vehicle['fwd'])
    ...
    def add_surface_to_areas_to_blit(self, surface, surface_pos):
        ...
        self.areas_to_blit.append([surface_pos[0], surface_pos[1], surface_pos[0] + surface_width, surface_pos[1] + surface_height])
    ...
    def menu(self):
        ...
        if self.ms:
            self.ms.update(ev)
            if self.ms.choice:
                self.choose(self.ms.choice_label[0])
        if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 3:
            self.ms.set(self.view,ev.pos)

    def choose(self,var):
        val = getattr(self,var)
        setattr(self,var,not val)
